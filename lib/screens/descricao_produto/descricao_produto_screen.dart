...
class DescricaoProdutoScreen extends StatefulWidget {
  ...
  @override
  State<DescricaoProdutoScreen> createState() => _DescricaoProdutoScreenState();
}
...
class _DescricaoProdutoScreenState extends State<DescricaoProdutoScreen> {
  ...
  void _init() {
    ...
    final produtoArg = args['produto'];

    if (produtoArg is ProdutoModel) {
      _produto = produtoArg;
    ...
    } else if (produtoArg is Map<String, dynamic>) {
      _produto = ProdutoModel.fromJson(produtoArg);
    } else {
      Navigator.of(context).pop();
      return;
    }
    ...
    if (_editar && args['produtoEditado'] != null) {
      final editado = args['produtoEditado'];
      _itemEditando = editado is CartItemModel ? editado : null;
  ...
  double get _extraAdicionais =>
      _adicionais.where((a) => a.selecionado).fold(0.0, (s, a) => s + a.preco);

  double get _precoTotal => (_produto?.price ?? 0) + _extraAdicionais;
  ...
  void _adicionarAoCarrinho() {
    ...
    final item = CartItemModel(
      ...
      adicionais: _adicionais
          ...
          .map(
            (a) => AdicionalSelecionado(id: a.id, nome: a.nome, preco: a.preco),
          )
          .toList(),
    );
            ...
            child: SingleChildScrollView(
              ...
              child: Column(
                ...
                children: [
                  ...
                  Text(
                    _produto!.description ?? 'Produto delicioso',
                    style: const TextStyle(
                      fontSize: 13,
    ...
    return Column(
      ...
      children: [
        ...
        if (subtitle != null)
          Text(
            subtitle,
            style: const TextStyle(fontSize: 12, color: AppColors.textEmpty),
          ),
        const SizedBox(height: 8),
      ],
    );
          ...
          child: Row(
            children: [
              ...
              Container(
                ...
                decoration: BoxDecoration(
                  color: ing.incluso ? AppColors.badgeBg : AppColors.redErrorBg,
                  borderRadius: BorderRadius.circular(20),
                ),
                child: Text(
        ...
        children: [
          Column(
            ...
            children: [
              ...
              if (_extraAdicionais > 0)
                Text(
                  '+R\$ ${_extraAdicionais.toStringAsFixed(2)} em adicionais',
                  style: const TextStyle(
                    fontSize: 11,
                    color: AppColors.textSection,
                  ),
          ...
          Expanded(
            child: ElevatedButton.icon(
              ...
              icon: Icon(_editar ? Icons.check : Icons.shopping_bag_outlined),
              label: Text(
                _editar ? 'Salvar' : 'Adicionar',
